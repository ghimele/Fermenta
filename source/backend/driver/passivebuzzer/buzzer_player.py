...
buzzer_pin = 24  #default pin number
...
def buzz(frequency, length):	 		#create the function "buzz" and feed it the pitch and duration)
	...
	for i in range(numCycles):			#start a loop from 0 to the variable "cycles" calculated above
		GPIO.output(buzzer_pin, True)	#set buzzer_pin to high
		time.sleep(delayValue)			#wait with buzzer_pin high
		GPIO.output(buzzer_pin, False)	#set buzzer_pin to low
...
if __name__ == '__main__':
	...
	if len(sys.argv) > 1:
		...
		buzzer_pin = int(args[0])
	print ("PIN:", buzzer_pin)				# Program start from here
	try:
		setup()
		print ("Fermenta melody")
		now = datetime.datetime.now()
		print(now.strftime('%Y-%m-%d %H:%M:%S'))
		play(Fermenta_melody, Fermenta_tempo, 0.50, 1.000)
		now = datetime.datetime.now()
		print(now.strftime('%Y-%m-%d %H:%M:%S'))
		destroy()
